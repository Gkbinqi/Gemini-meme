...
图片由定制 Gem 制作的生成器生成。
如果你想自己生成类似风格的图片，可以参考 `/prompts` 目录下的文档。
模型使用的是 Nano Banana。你可以直接复制文档中的系统指令（Instruction）作为基础设定，并结合具体的提示词进行生成或修改。
...
2.  **商标与知识产权免责声明 (Trademark Disclaimer) - 核心约束:**
    ...
    * **本项目与 Google 没有任何官方关联、赞助或认可。**
    * **风险提示：** 尽管图像本身以 CC0 释出，但由于其包含了可能指向真实企业商标的视觉元素，**强烈建议不要将这些图像用于任何可能侵犯 Google 官方权益的商业盈利用途，或将其伪装成官方产品进行欺诈行为。** 如因不当使用造成的任何商标侵权纠纷，由使用者自行承担全部法律责任。
